...
public class contractSignPage extends BasePage {
	String path=System.getProperty("user.dir")+"\\upload\\Contract.exe";
	...
	public void signContractListPage() throws Exception{
		this.click(new Locator("上传图片"));
		this.sleep(500);
		AutoUI AU=new AutoUI();
		driver.switchTo().frame(this.findElement(driver, new Locator("frame")));
		this.click(new Locator("合同批量选择文件"));
		this.sleep(2000);
		AU.UploadImage(path);
		this.sleep(4000);
		this.click(new Locator("合同开始上传"));
		driver.switchTo().parentFrame();
		this.sleep(1000);
		this.click(new Locator("合同上传图片提交"));
		this.sleep(1000);
		this.click(new Locator("上传成功"));
		this.sleep(1000);
		this.click(new Locator("GPS确认"));
		this.sleep(1000);
		this.click(new Locator("GPS确认2"));
		this.sleep(500);
		this.click(new Locator("新增GPS确定"));
		this.sleep(1000);
